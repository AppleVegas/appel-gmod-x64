...
public:
	...
	void StartDrawing()
	{
		...
		if (!StartDrawing)
			StartDrawing = (StartDrawingFn)(Util::Pattern::FindPattern("vguimatsurface.dll", "55 8B EC 64 A1 ?? ?? ?? ?? 6A FF 68 ?? ?? ?? ?? 50 64 89 25 ?? ?? ?? ?? 83 EC 14")); //A1 20 1E //83 EC 14
	...
	void FinishDrawing()
	{
		...
		if (!FinishDrawing)
			FinishDrawing = (FinishDrawingFn)(Util::Pattern::FindPattern("vguimatsurface.dll", "55 8B EC 6A FF 68 ?? ?? ?? ?? 64 A1 ?? ?? ?? ?? 50 64 89 25 ?? ?? ?? ?? 51 56 6A 00")); //51 56 8B F1 //51 56 6A 00
...
//{
//public:
//	BYTE pad01[4]; vmtable
//	BYTE pad02[4]; vmtable
//public:
//	CViewSetup m_view;
//};